...
def quick_probe(query: str, profile=None, *,
    ...
    return pipeline_(
        search(query, profile=profile, page_size=page_size,
               max_results=max_results),
        filter(screen(profile=profile, min_survivors=min_survivors)),
    )


def quick_discovery(query: str, profile=None, *,
                    page_size: int = 100, max_results: int = 200,
    ...
    p = quick_probe(query, profile, page_size=page_size,
...
def harvest(query: str, profile=None, *,
    ...
    p = full_report(query, profile, top=top, page_size=page_size,
                    max_results=max_results, min_survivors=min_survivors,
                    workers=workers, cache_root=cache_root)
