...
- **A degraded inspect is not cached**: if any candidate stayed shallow
  (transient failure), the result is logged and skipped — re-runs re-inspect
  instead of replaying stale shallow data. Detect stragglers via
  `candidate.inspection_depth != "deep"` / `InspectResult.fmt()`;
  `run(force=True)` (or a fresh input key) bypasses any other cached entry.
